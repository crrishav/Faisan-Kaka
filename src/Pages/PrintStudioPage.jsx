...
const PrintStudioPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f5f5f5] overflow-x-hidden">
          ...
          <motion.div variants={panelIn} className="mt-5 lg:hidden flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="ml-auto flex gap-1.5">
                {['front','back'].map((s) => (
                  <PillBtn key={s} active={activeSide === s} onClick={() => setActiveSide(s)} className="capitalize">{s}</PillBtn>
                ))}
              </div>
            </div>
            {isJeans && (
            ...
            {/* ── Canvas preview ── */}
            <div className="rounded-[20px] border border-black/10 bg-[#ececec] overflow-hidden shadow-[0_8px_22px_rgba(0,0,0,0.07)]">
              {/* canvas info bar */}
              <div className="flex items-center justify-between px-3 pt-3 pb-2">
                <div className="flex items-center gap-2">
                  <p className="text-xs font-black text-black/60">
                    {garmentName} · <span className="capitalize">{activeSide}</span>
                    {!isJeans ? <span className="inline-flex items-center gap-1 ml-1">· <span className="w-3 h-3 rounded-sm inline-block border border-black/15" style={{ background: garmentColor }} /> {garmentColor.toUpperCase()}</span> : ''}
                  </p>
                </div>
                <div className="flex items-center gap-1.5">
                  {/* If a design is selected on this side, show quick delete */}
                  {activeArtworkOnThisSide && (
                    <button
                      onClick={() => { removeArtworkFromSide(activeArtworkOnThisSide.id, activeSide); showToast(`Removed from ${activeSide}`); }}
                      className="flex items-center gap-1 px-2.5 h-7 rounded-lg bg-red-500/90 text-white text-[11px] font-black cursor-pointer active:opacity-75 transition">
                      <Icons.Trash /> Remove
                    </button>
                  )}
                  <button onClick={() => { resetView(); setIsFullscreen(true); }}
                    className="flex items-center gap-1 px-2.5 h-7 rounded-lg bg-black text-white text-[11px] font-black cursor-pointer active:opacity-75 transition">
                    <Icons.Maximize /> Edit
                  </button>
                </div>
              </div>
              ...
              <MockupCanvas {...canvasProps}
                ...
                emptyLabel="Upload a design to preview"
              />

              {/* design quick-select strip — horizontal scroll */}
              {artworks.length > 0 && (
                <div className="flex gap-2 px-3 py-2.5 overflow-x-auto no-scrollbar">
                  {artworks.map((aw) => {
                    ...
                    return (
                      <button key={aw.id} onClick={() => setActiveArtworkId(aw.id)}
                        className={`shrink-0 w-11 h-11 rounded-xl border-2 overflow-hidden transition-all cursor-pointer relative
                          ${isAct ? 'border-black shadow-md scale-105' : 'border-white/50 opacity-60 hover:opacity-90'}`}>
                        {aw.previewUrl
                          ? <img src={aw.previewUrl} alt="" className="w-full h-full object-cover" />
                          : <div className="w-full h-full bg-black/30 flex items-center justify-center"><span className="text-[8px] font-black text-white">PDF</span></div>
                        }
                        {/* dim overlay if not visible on this side */}
                        {!onThisSide && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <span className="text-[8px] text-white font-black">OFF</span>
                          </div>
                        )}
                      </button>
                    );
                  ...
                  {/* add more button */}
                  {artworks.length < MAX_UPLOADS && (
                    <button onClick={() => inputRef.current?.click()}
                      className="shrink-0 w-11 h-11 rounded-xl border-2 border-dashed border-black/20 bg-black/5 flex items-center justify-center cursor-pointer hover:bg-black/8 transition">
                      <span className="text-black/40 text-lg font-bold leading-none">+</span>
                    </button>
                  )}
                </div>
              )}
            ...
            {/* ── Mobile tabbed control panel ── */}
            <div className="rounded-[20px] border border-black/10 bg-white/90 backdrop-blur-sm shadow-[0_8px_22px_rgba(0,0,0,0.07)] overflow-hidden">
              {/* Tab bar */}
              <div className="grid grid-cols-4 border-b border-black/8">
                {[
                  ...
                  <button key={id} onClick={() => setMobileTab(id)}
                    className={`py-3 text-[11px] font-black uppercase tracking-[0.08em] transition cursor-pointer relative
                      ${mobileTab === id ? 'text-black' : 'text-black/35 hover:text-black/60'}`}>
                    {label}
                    {mobileTab === id && (
                      <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-black rounded-full" />
                    )}
                  </button>
                ))}
              </div>
              ...
              <AnimatePresence mode="wait">
                ...
                {/* ── DESIGNS TAB ── */}
                {mobileTab === 'designs' && (
                  <motion.div key="designs" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }} className="p-4">
                    {/* upload zone */}
                    <div
                      onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                      onDragLeave={() => setDragOver(false)}
                      onDrop={handleDrop}
                      onClick={() => inputRef.current?.click()}
                      className={`rounded-xl border-2 border-dashed h-16 flex items-center justify-center gap-3 cursor-pointer transition-all active:scale-[0.98]
                        ${dragOver ? 'border-black/50 bg-black/4' : 'border-black/15 bg-black/2'}`}>
                      <div className="text-black/30 scale-75"><Icons.Upload /></div>
                      <div>
                        <p className="text-sm font-bold text-black leading-tight">Tap to upload</p>
                        <p className="text-[10px] text-black/40">PNG · SVG · JPG · PDF · max 10MB</p>
                      </div>
                      <span className="ml-auto mr-3 text-xs font-black text-black/25">{artworks.length}/{MAX_UPLOADS}</span>
                    </div>
                    ...
                    <div className="mt-3 flex flex-col gap-1.5">
                      {artworks.length === 0 && (
                        <p className="text-xs text-black/35 text-center py-5">No designs yet — upload one above</p>
                      )}
                      {artworks.map((aw, i) => {
                        const isAct = aw.id === activeArtworkId;
                        ...
                        return (
                          <div key={aw.id}
                            onClick={() => setActiveArtworkId(aw.id)}
                            className={`flex items-center gap-2.5 rounded-xl border px-3 py-2.5 cursor-pointer transition-all active:scale-[0.99]
                              ${isAct ? 'border-black bg-black text-white' : 'border-black/10 bg-white'}`}>
                            {/* thumbnail */}
                            {aw.previewUrl
                              ? <img src={aw.previewUrl} alt="" className="w-9 h-9 rounded-lg object-cover shrink-0 bg-black/8" />
                              : <div className={`w-9 h-9 rounded-lg shrink-0 flex items-center justify-center ${isAct ? 'bg-white/15' : 'bg-black/10'}`}><span className={`text-[9px] font-black ${isAct ? 'text-white/60' : 'text-black/40'}`}>PDF</span></div>
                            }
                            {/* info */}
                            <div className="flex-1 min-w-0">
                              <p className={`text-[10px] font-black uppercase tracking-wide mb-0.5 ${isAct ? 'text-white/45' : 'text-black/35'}`}>Design {i + 1}</p>
                              <p className="text-xs font-bold truncate">{aw.name}</p>
                              {/* side indicators */}
                              <div className="flex gap-1 mt-1">
                                {[['F', onFront], ['B', onBack]].map(([label, active]) => (
                                  <span key={label} className={`text-[9px] font-black px-1.5 py-0.5 rounded-md
                                    ${active
                                      ? (isAct ? 'bg-white/20 text-white' : 'bg-black/10 text-black/60')
                                      : (isAct ? 'bg-white/8 text-white/25' : 'bg-black/4 text-black/20 line-through')}`}>
                                    {label}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex gap-1 shrink-0">
                              <button onClick={(e) => { e.stopPropagation(); duplicateArtwork(aw.id); }}
                                title="Duplicate"
                                className={`w-8 h-8 rounded-lg flex items-center justify-center transition cursor-pointer active:scale-90
                                  ${isAct ? 'text-white/60 hover:text-white hover:bg-white/15' : 'text-black/30 hover:text-black hover:bg-black/8'}`}>
                                <Icons.Copy />
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); removeArtworkFromSide(aw.id, activeSide); showToast(`Removed from ${activeSide}`); }}
                                title={`Remove from ${activeSide}`}
                                className={`w-8 h-8 rounded-lg flex items-center justify-center transition cursor-pointer active:scale-90
                                  ${isAct ? 'text-red-300 hover:text-red-200 hover:bg-white/10' : 'text-red-400 hover:text-red-600 hover:bg-red-50'}`}>
                                <Icons.Trash />
                              </button>
                            </div>
                          </div>
                    ...
                    {/* delete all from side — only if any designs exist on this side */}
                    {sideArtworks.length > 0 && (
                      <button onClick={() => {
                        sideArtworks.forEach((a) => removeArtworkFromSide(a.id, activeSide));
                        showToast(`Cleared all from ${activeSide}`);
                      }}
                        className="mt-3 w-full h-9 rounded-xl border border-red-200 text-red-500 text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-red-50 transition cursor-pointer active:scale-[0.98]">
                        <Icons.Trash /> Clear {activeSide} side
                      </button>
                    )}
                  </motion.div>
                )}
                ...
                {/* ── COLOR TAB ── */}
                {mobileTab === 'color' && (
                  <motion.div key="color" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }} className="p-4">
                    {isJeans ? (
                      <div className="py-4 text-center">
                        <p className="text-xs text-black/40 mb-3">Jeans color is fixed — switch the wash style above.</p>
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <div className="w-12 h-12 rounded-xl border border-black/12 shadow-sm shrink-0 transition-all" style={{ background: garmentColor }} />
                          <div className="flex-1">
                            <p className="text-[10px] font-black uppercase tracking-wide text-black/40 mb-1">Hex Code</p>
                            <input type="text" value={colorInput}
                              onChange={(e) => { setColorInput(e.target.value); setColorError(''); }}
                              onBlur={() => commitColor(colorInput.trim())}
                              onKeyDown={(e) => e.key === 'Enter' && commitColor(colorInput.trim())}
                              placeholder="#111111"
                              className="w-full h-9 rounded-xl border border-black/16 bg-white px-3 text-sm font-bold outline-none focus:border-black/45 transition" />
                          </div>
                          <input type="color" value={garmentColor}
                            onChange={(e) => applyPreset(colord(e.target.value).toHex())}
                            className="h-12 w-12 rounded-xl border border-black/16 bg-white p-1.5 cursor-pointer shrink-0" />
                        </div>
                        ...
                        <div className="grid grid-cols-6 gap-2">
                          {COLOR_PRESETS.map(({ hex, label }) => (
                            <button key={hex} type="button" title={label} onClick={() => applyPreset(hex)}
                              className={`aspect-square rounded-xl border-2 transition-all cursor-pointer relative active:scale-95
                                ${garmentColor === hex ? 'border-black scale-105' : 'border-transparent'}`}
                              style={{ background: hex, boxShadow: '0 1px 3px rgba(0,0,0,0.15)' }}>
                              {garmentColor === hex && (
                                <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                  <span className={`text-[10px] font-black ${['#FFFFFF','#D9D9D9'].includes(hex) ? 'text-black' : 'text-white'}`}>✓</span>
                                </span>
                              )}
                            </button>
                          ))}
                        ...
                        <button onClick={() => applyPreset('#111111')}
                          className="mt-3 w-full h-9 rounded-xl bg-black/5 text-black text-xs font-bold flex items-center justify-center hover:bg-black/10 transition cursor-pointer active:scale-[0.98]">
                ...
                {/* ── ADJUST TAB ── */}
                {mobileTab === 'adjust' && (
                  <motion.div key="adjust" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }} className="p-4">
                    {!activeArtworkOnThisSide ? (
                      <div className="py-5 text-center">
                        <p className="text-xs text-black/40 mb-1">No design selected on this side</p>
                        <p className="text-[11px] text-black/25">Upload a design or switch sides</p>
                      </div>
                    ) : (
                      <>
                        {/* selected design info */}
                        <div className="flex items-center gap-2.5 mb-3 rounded-xl bg-black/4 px-3 py-2.5">
                          {activeArtworkOnThisSide.previewUrl
                            ? <img src={activeArtworkOnThisSide.previewUrl} alt="" className="w-8 h-8 rounded-lg object-cover shrink-0 bg-black/8" />
                            : <div className="w-8 h-8 rounded-lg bg-black/15 flex items-center justify-center shrink-0"><span className="text-[9px] font-black text-black/40">PDF</span></div>
                          }
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-black uppercase tracking-wide text-black/40">Editing</p>
                            <p className="text-xs font-bold truncate">{activeArtworkOnThisSide.name}</p>
                          </div>
                          ...
                          <button onClick={() => { removeArtworkFromSide(activeArtworkOnThisSide.id, activeSide); showToast(`Removed from ${activeSide}`); }}
                            className="flex items-center gap-1 px-2.5 h-7 rounded-lg bg-red-500 text-white text-[11px] font-black cursor-pointer active:opacity-75 transition shrink-0">
                            <Icons.Trash /> Remove
                          </button>
                        </div>

                        {/* transform controls — 2 per row, bigger touch targets */}
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            ...
                            <button key={label} onClick={fn}
                              className="h-11 rounded-xl bg-black/6 text-black/70 text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-black/12 transition cursor-pointer active:scale-[0.97]">
                              {icon}{label}
                            </button>
                          ))}
                        </div>
                        ...
                        <div className="mt-3">
                          <div className="flex justify-between mb-2">
                            <p className="text-[11px] font-black uppercase tracking-wide text-black/45 flex items-center gap-1"><Icons.Opacity /> Opacity</p>
                            <span className="text-xs font-bold text-black/55">{Math.round((activeArtworkOnThisSide.opacity ?? 1) * 100)}%</span>
                          </div>
                          <input type="range" min={10} max={100} step={5}
                            ...
                            className="w-full accent-black h-2 cursor-pointer rounded-full" />
                        </div>

                        {/* duplicate */}
                        <button onClick={() => duplicateArtwork(activeArtworkOnThisSide.id)}
                          className="mt-3 w-full h-10 rounded-xl bg-black/6 text-black text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-black/12 transition cursor-pointer active:scale-[0.98]">
                          <Icons.Copy /> Duplicate Design
                        </button>

                        {/* also remove from all sides */}
                        <button onClick={() => { removeArtwork(activeArtworkOnThisSide.id); showToast('Design deleted'); }}
                          className="mt-2 w-full h-10 rounded-xl border border-red-200 text-red-500 text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-red-50 transition cursor-pointer active:scale-[0.98]">
                          <Icons.Trash /> Delete from All Sides
                        </button>
                      </>
                    )}
                  </motion.div>
                ...
                {/* ── LAYERS TAB ── */}
                {mobileTab === 'layers' && (
                  <motion.div key="layers" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }} className="p-4">
                    {artworks.length === 0 ? (
                      <p className="text-xs text-black/35 text-center py-5">No designs yet</p>
                    ) : (
                      <>
                        <p className="text-[10px] font-black uppercase tracking-wide text-black/40 mb-2">
                          Layer order · <span className="capitalize">{activeSide}</span> side (top = front)
                        </p>
                            ...
                            return (
                              <div key={aw.id}
                                ...
                                className={`flex items-center gap-2.5 rounded-xl border px-3 py-2 cursor-pointer transition-all
                                  ${isAct ? 'border-black bg-black text-white' : onThisSide ? 'border-black/10 bg-white' : 'border-black/5 bg-black/3 opacity-50'}`}>
                                <span className={`text-[10px] font-black w-5 text-center ${isAct ? 'text-white/40' : 'text-black/30'}`}>
                                  {artworks.length - revI}
                                </span>
                                {aw.previewUrl
                                  ? <img src={aw.previewUrl} alt="" className="w-7 h-7 rounded-lg object-cover shrink-0 bg-black/8" />
                                  : <div className="w-7 h-7 rounded-lg bg-black/20 flex items-center justify-center shrink-0"><span className="text-[8px] font-black text-white/60">PDF</span></div>
                                }
                                <p className="flex-1 text-xs font-bold truncate">{aw.name}</p>
                                {/* toggle visibility on this side */}
                                <button onClick={(e) => {
                                  e.stopPropagation();
                                  if (!onThisSide) {
                                    // re-enable on this side
                                    setArtworks((p) => p.map((a) => a.id !== aw.id ? a : { ...a, visible: { ...a.visible, [activeSide]: true } }));
                                    showToast(`Shown on ${activeSide}`);
                                  } else {
                                    removeArtworkFromSide(aw.id, activeSide);
                                    showToast(`Hidden on ${activeSide}`);
                                  }
                                }}
                                  title={onThisSide ? 'Hide on this side' : 'Show on this side'}
                                  className={`w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-black transition cursor-pointer active:scale-90
                                    ${isAct ? 'text-white/60 hover:text-white hover:bg-white/15' : 'text-black/30 hover:text-black hover:bg-black/8'}`}>
                                  {onThisSide ? '●' : '○'}
                                </button>
                                {/* layer order buttons */}
                                <div className="flex flex-col gap-0.5">
                                  <button onClick={(e) => { e.stopPropagation(); moveLayer(aw.id, 'up'); }}
                                    disabled={realIdx === artworks.length - 1}
                                    className={`w-5 h-4 flex items-center justify-center rounded transition cursor-pointer
                                      ${isAct ? 'text-white/60 hover:text-white' : 'text-black/25 hover:text-black'}
                                      disabled:opacity-20 disabled:cursor-not-allowed`}>
                                    <Icons.LayerUp />
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); moveLayer(aw.id, 'down'); }}
                                    disabled={realIdx === 0}
                                    className={`w-5 h-4 flex items-center justify-center rounded transition cursor-pointer
                                      ${isAct ? 'text-white/60 hover:text-white' : 'text-black/25 hover:text-black'}
                                      disabled:opacity-20 disabled:cursor-not-allowed`}>
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 backdrop-blur-xl border-t border-black/8 px-4 py-3 shadow-[0_-6px_20px_rgba(0,0,0,0.06)]">
        <div className="flex gap-2.5 items-center">
          {/* upload */}
          <button onClick={() => inputRef.current?.click()}
            className="flex-1 h-11 rounded-xl bg-black/7 text-black text-xs font-black flex items-center justify-center gap-1.5 active:bg-black/14 transition cursor-pointer">
            <Icons.Upload /> Upload
          </button>
          {/* order */}
          <button disabled={!hasUploads} onClick={() => setShowOrder(true)}
            className={`flex-1 h-11 rounded-xl text-xs font-black transition flex items-center justify-center gap-1.5
              ${hasUploads ? 'bg-black text-white active:opacity-80 cursor-pointer' : 'bg-black/10 text-black/30 cursor-not-allowed'}`}>
            Print Order →
          </button>
        </div>
