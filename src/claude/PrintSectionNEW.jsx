...
const PrintSection = () => {
  ...
  const { setPendingFile } = usePrintContext();
  ...
  const handleDragLeave = useCallback((e) => {
    e.preventDefault();
    // Only clear drag state when leaving the drop zone entirely (not when moving over a child element)
          ...
          <AnimatePresence>
            {uploadedFile && (
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={() => { setPendingFile(uploadedFile); navigate('/print'); }}
              >
                Proceed to Print →
              </motion.button>
            )}
