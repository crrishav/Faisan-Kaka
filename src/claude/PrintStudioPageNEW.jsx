...
const PrintStudioPage = () => {
  ...
  const showToast = (msg) => { setToast(msg); setTimeout(() => setToast(''), msg === 'coming-soon' ? 3200 : 2600); };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f5f5f5] overflow-x-hidden">
      ...
      <AnimatePresence>
        ...
        {toast === 'coming-soon' && (
          <motion.div key="coming-soon-toast"
            initial={{ opacity: 0, y: -14, scale: 0.92 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -14, scale: 0.92 }}
            transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
            className="fixed top-5 left-1/2 -translate-x-1/2 z-[200] pointer-events-none flex items-center gap-2 bg-white border border-black/10 shadow-[0_8px_24px_rgba(0,0,0,0.12)] text-black text-xs font-black px-4 py-2.5 rounded-full">
            <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse shrink-0" />
            Coming Soon — Custom printing is on the way!
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <motion.div variants={panelIn} className="mt-8 hidden lg:grid grid-cols-[390px_1fr] gap-5 items-start">
            ...
            <div className="flex flex-col gap-4">
              ...
              <button type="button" onClick={handleAddToCart}
                className="w-full h-12 rounded-2xl font-black text-sm transition-all cursor-pointer bg-white border-2 border-black/10 text-black/60 hover:border-amber-400 hover:text-amber-600 hover:bg-amber-50 shadow-sm flex items-center justify-center gap-2">
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-[100] bg-white border-t border-black/5 pb-safe pt-2 px-3 shadow-[0_-12px_40px_rgba(0,0,0,0.08)]">
        <div className="flex gap-2 items-center h-14 pb-2">
          ...
          <button onClick={handleAddToCart}
            className="flex-[1.5] h-full rounded-2xl flex flex-col items-center justify-center transition-all gap-0.5 bg-white border-2 border-black/8 text-black/55 hover:border-amber-400 hover:text-amber-600 active:scale-95">
            <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" />
            <span className="text-[11px] font-black leading-tight">Coming Soon</span>
          </button>
